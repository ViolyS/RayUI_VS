...
local function LoadCurrency()
    ...
    local function formatMoney(money, icon)
        ...
        if R.global.general.numberType == 1 then
            if gold ~= 0 then
                if icon then
                    return format(BreakUpLargeNumbers(gold)..GOLD_AMOUNT_TEXTURE:gsub("%%d",""))
                else
                    return format("%s".."|cffffd700G|r", R:ShortValue(gold))
                end
            elseif silver ~= 0 then
                if icon then
                    return format(silver..SILVER_AMOUNT_TEXTURE:gsub("%%d",""))
                else
                    return format("%s".."|cffc7c7cfS|r", silver)
                end
            else
                if icon then
                    return format(copper..COPPER_AMOUNT_TEXTURE:gsub("%%d",""))
                else
            ...
            if gold ~= 0 then
                return format("%s".."|cffffd700金|r", R:ShortValue(gold))
            elseif silver ~= 0 then
                return format("%s".."|cffc7c7cf银|r", silver)
            else
                return format("%s".."|cffeda55f铜|r", copper)
            end
        end
    end
