...
local function SkinDBM()
	...
	local buttonsize = 20
	local difv = 12			-- buttonsize与frame的差值，取值在0-20之间，最好不要大于18。
	...
	local function ApplyMyStyle(self)
		...
		self.options.BarYOffset = 6 + difv
		self.options.HugeBarYOffset = 6+ difv
		self.options.ExpandUpwards = false
		if self.mainAnchor then
			self.mainAnchor:ClearAllPoints()
			self.mainAnchor:SetPoint("TOPLEFT", Minimap, "TOPRIGHT", self.options.Width/2 + 35 + buttonsize, self.options.BarYOffset - difv)
		end
			...
			if not bar.injected then
				function bar:ApplyStyle()
					...
					if not (icon1.overlay) then
						...
						icon1.overlay:CreatePanel(template, buttonsize, buttonsize, "BOTTOMRIGHT", tbar, "BOTTOMLEFT", - 4, 0)
					...
					if not frame.styled then
						...
						frame:SetHeight(buttonsize - difv)
					...
					if not name.styled then
						name:ClearAllPoints()
						name:Point("BOTTOMLEFT", frame, "BOTTOMLEFT", 4, 4)
						name:SetWidth(165)
						name:SetHeight(8)
					...
					if not timer.styled then
						...
						timer:Point("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -4, 4)
	...
	local ForceOptions = function()
		...
		db.HugeScale = 1
		db.BarXOffset = 0
		db.BarYOffset = 6 + difv
		db.HugeBarYOffset = 6 + difv
